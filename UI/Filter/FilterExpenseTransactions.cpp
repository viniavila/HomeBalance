...
#include "ui_FilterExpenseTransactions.h"
#include <QLocale>

class FilterExpenseTransactionsPrivate {
public:
    FilterExpenseTransactions * const parent;
    Ui::FilterExpenseTransactions * const ui;

    FilterExpenseTransactionsPrivate(FilterExpenseTransactions * p, Ui::FilterExpenseTransactions * ui) :
        parent(p),
        ui(ui)
    {
    ...
    void btnReset_onClicked() {
        ...
        ui->txtGroup->setText(QString());
    }
};

FilterExpenseTransactions::FilterExpenseTransactions(QWidget *parent) :
    AbstractFilterDialog(parent),
    ui(new Ui::FilterExpenseTransactions),
    d(new FilterExpenseTransactionsPrivate(this, ui))
{
    ui->setupUi(this);
    d->btnReset_onClicked();
    connect(ui->btnReset, &QAbstractButton::clicked, this, std::bind(&FilterExpenseTransactionsPrivate::btnReset_onClicked, d));
}

FilterExpenseTransactions::~FilterExpenseTransactions() {
    delete d;
    delete ui;
}

void FilterExpenseTransactions::setFilterList(const HBDataModelFilterList& filter_list) {
    QLocale l;
    d->btnReset_onClicked();
    for (const HBDataModelFilter& filter : filter_list) {
        ...
        if (filter.column == 5 && filter.type == HBDataModelFilter::FILTER_TEXT) {
            ui->chkGroup->setChecked(true);
            ui->txtGroup->setText(filter.value.toString());
        }
        ...
        if (filter.column == 7 && filter.type == HBDataModelFilter::FILTER_TEXT) {
            ui->chkDescripton->setChecked(true);
            ui->txtDescription->setText(filter.value.toString());
        }
        if (filter.column == 8 && filter.type == HBDataModelFilter::FILTER_GE_FLOAT) {
            ui->chkBeginValue->setChecked(true);
            ui->txtBeginValue->setText(l.toString(filter.value.toFloat(), 'f', 2));
        }
        if (filter.column == 8 && filter.type == HBDataModelFilter::FILTER_LE_FLOAT) {
...
HBDataModelFilterList FilterExpenseTransactions::filterList() const {
    ...
    if (ui->chkGroup->isChecked())
        list << HBDataModelFilter(5, HBDataModelFilter::FILTER_TEXT, ui->txtGroup->text());
    if (ui->chkCategory->isChecked())
        list << HBDataModelFilter(6, HBDataModelFilter::FILTER_TEXT, ui->txtCategory->text());
    if (ui->chkDescripton->isChecked())
        list << HBDataModelFilter(7, HBDataModelFilter::FILTER_TEXT, ui->txtDescription->text());
    if (ui->chkBeginValue->isChecked())
        list << HBDataModelFilter(8, HBDataModelFilter::FILTER_GE_FLOAT, l.toFloat(ui->txtBeginValue->text()));
    if (ui->chkEndValue->isChecked())
        list << HBDataModelFilter(8, HBDataModelFilter::FILTER_LE_FLOAT, l.toFloat(ui->txtEndValue->text()));
    return list;
}
